<template>
    <section class="barrage-wrapper" ref="barrageWrapper" :style="{ height: percent + '%', opacity: isPause ? 0 : 1 }">
        <div class="barrage-main">
            <div class="barrage-main-dm" :class="{ 'ani-pause': isPause, 'ani-running': !isPause }" ref="barrageMainDm">
            </div>
        </div>
    </section>
</template>
...
export default {
    ...
    methods: {
        handleSizeChange() {
            this.barMainWidth = this.barrageMainDm.clientWidth;
            const items = document.getElementsByClassName('barrage-item')
            for (let i = 0; i < items.length; i++) {
                items[i].style.transform = `translate3d(${this.barMainWidth}px,0,0)`
            }
        },
        visibilitychangeFn() {
            if (!document.hidden) {
